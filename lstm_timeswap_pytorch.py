...
if isinstance(normalized_deforest_2017_2021, pd.DataFrame):
    ...
    all_targets = np.concatenate(all_targets, axis=0)  # Concatenate along the sample dimension
...
# Modify augment_data function to use random time swap instead of time shift
def augment_data_with_time_swap_biweeks(X_train, y_train, bin_indices, max_bin_size, num_months=6):
    augmented_X = []
    augmented_y = []
    ...
    for bin_idx in range(len(np.bincount(bin_indices))):
        ...
        if num_samples_needed > 0:
            ...
            augmented_X.append(np.concatenate([bin_X, sampled_X, swapped_X], axis=0)[:max_bin_size])
            augmented_y.append(np.concatenate([bin_y, sampled_y, sampled_y], axis=0)[:max_bin_size])
        else:
...
X_train_augmented, y_train_augmented = augment_data_with_time_swap_biweeks(X_train, y_train, bin_indices, max_bin_size, num_months=6)
...
torch.save(model.state_dict(), 'D:/ITC/Thesis/Scripts/results/results/models/lstm_loss_pytorch_timeswap.pth')